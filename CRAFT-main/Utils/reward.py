...
def getReward(name):
    if name == "QED":
        return QEDReward()
    if name == "AKT1":
        with open('/media/aita4090/YRF/CRAFT-main/Model/QSAR/qsar_AKT1_optimized.pkl', mode='rb') as f:
            qwar_model = pickle.load(f)
        return QSAR_Reward(qwar_model)
    if name == "CXCR4":
        with open('/media/aita4090/YRF/CRAFT-main/Model/QSAR/qsar_CXCR4_optimized.pkl', mode='rb') as f:
            qwar_model = pickle.load(f)
        return QSAR_Reward(qwar_model)
    if name == "DRD2":
        with open('/media/aita4090/YRF/CRAFT-main/Model/QSAR/qsar_DRD2_optimized.pkl', mode='rb') as f:
            qwar_model = pickle.load(f)
        return QSAR_Reward(qwar_model)
